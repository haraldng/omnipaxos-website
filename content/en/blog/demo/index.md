...
### Handling Requests
Since the database will be replicated, the servers cannot directly apply changes to their database when a client request is received. The database can become inconsistent if the servers apply concurrent modifications in different order or if some server fails. Instead, the changes to the database must first be committed in the OmniPaxos log which will provide a single order for all servers and guarantee that any committed entry have been replicated on at least a majority of servers.
...
We have now updated our servers to append client requests to the `OmniPaxos` log and update the database *when* entries have been committed. But to drive the progress and for entries to actually get committed, each server must send out the messages that are generated by their `OmniPaxos` struct. We must therefore periodically fetch the outgoing messages from `OmniPaxos` and send them via our network implementation.
